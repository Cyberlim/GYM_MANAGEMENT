...
class _MembersPageState extends ConsumerState<MembersPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final highlightId = GoRouterState.of(
      context,
    ).uri.queryParameters['highlightId'];
            ...
            if (!isMobile)
              Container(
                ...
                child: ToggleButtons(
                  ...
                  onPressed: (index) {
                    ref
                        .read(isMemberListViewProvider.notifier)
                        .setMode(index == 0);
                  },
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            flex: 3,
            child: Text('NAME & EMAIL', style: _headerStyle(context)),
          ),
          ...
          Expanded(
            flex: 2,
            child: Text('JOIN DATE', style: _headerStyle(context)),
          ),
...
void showAddMemberDialog(
  ...
  final emailController = TextEditingController(
    text: memberToEdit?.email ?? '',
  );
  final phoneController = TextEditingController(
    text: memberToEdit?.phone ?? '',
  );
  final addressController = TextEditingController(
    text: memberToEdit?.address ?? '',
  );
  ...
  DateTime selectedExpiryDate =
      memberToEdit?.expiryDate ?? DateTime.now().add(const Duration(days: 30));
  ...
  final TransformationController docTransformController =
      TransformationController();
      ...
      return StatefulBuilder(
        builder: (context, setState) {
          return AlertDialog(
            backgroundColor: Theme.of(context).colorScheme.surface,
            title: Text(
              memberToEdit == null ? 'Add New Member' : 'Edit Member',
              style: TextStyle(color: Theme.of(context).colorScheme.onSurface),
            ),
            content: SizedBox(
              width: 600,
                                ...
                                return plansAsync.when(
                                  ...
                                  data: (plans) {
                                    ...
                                    if (selectedPlan.isNotEmpty &&
                                        !planNames.contains(selectedPlan)) {
                              ...
                              return trainersAsync.when(
                                ...
                                data: (trainers) {
                                  return DropdownButtonFormField<String?>(
                                    ...
                                    onChanged: (val) =>
                                        setState(() => selectedTrainerId = val),
                                  );
                                },
                              );
                            },
                          ...
                          buildResponsiveRow(
                            InkWell(
                              onTap: () async {
                                ...
                                if (date != null)
                                  setState(() => selectedJoinDate = date);
                              },
                              child: InputDecorator(
                                decoration: InputDecoration(
                                  labelText: 'Join Date',
                                ...
                                child: Text(
                                  '${selectedJoinDate.year}-${selectedJoinDate.month.toString().padLeft(2, '0')}-${selectedJoinDate.day.toString().padLeft(2, '0')}',
                                  maxLines: 1,
                                  overflow: TextOverflow.ellipsis,
                                ),
                              ),
            ...
            actions: [
              ...
              isUploading
                  ...
                  : ElevatedButton(
                        ...
                        if (nameController.text.isNotEmpty &&
                          ...
                          try {
                            ...
                            if (newImageFile != null) {
                              ...
                              final uploadedUrl = await api.uploadFile(
                                bytes,
                                newImageFile!.name,
                              );
                              if (uploadedUrl != null) {
                                ...
                                if (memberToEdit != null &&
                                    memberToEdit.imageUrl != null &&
                                    memberToEdit.imageUrl!.startsWith('http') &&
                                    !memberToEdit.imageUrl!.contains(
                                      'unsplash.com',
                                    )) {
                                  try {
                            ...
                            if (newDocumentFile != null) {
                              final bytes = await newDocumentFile!
                                  .readAsBytes();
                              final uploadedUrl = await api.uploadFile(
                                bytes,
                              ...
                              if (uploadedUrl != null) {
                                ...
                                if (memberToEdit != null &&
                                    memberToEdit.documentUrl != null &&
                                    memberToEdit.documentUrl!.startsWith(
                                      'http',
                                    )) {
                                  try {
                                    await api.deleteFile(
                                      memberToEdit.documentUrl!,
                                    );
                                  } catch (e) {
                                    debugPrint(
                                      'Failed to delete old document: $e',
                                    );
                                  }
                                }
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Please fill all required fields including Date of Birth.',
                              ),
                      ...
                      child: Text(
                        memberToEdit == null ? 'Save Member' : 'Update Member',
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    Expanded(
                      child: _buildMemberInfoItem(
                        ...
                        Text(
                          member.dob != null
                              ? DateFormat('MMM d, yyyy').format(member.dob!)
                              : 'N/A',
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.bold,
                    ...
                    Expanded(
                      child: _buildMemberInfoItem(
                        ...
                        'Address',
                        Text(
                          member.address.isEmpty ? 'N/A' : member.address,
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.bold,
                            color: Theme.of(context).colorScheme.onSurface,
                  ...
                  children: [
                    Expanded(
                      child: Consumer(
                        builder: (context, ref, child) {
                          final trainers =
                              ref.read(trainersProvider).value ?? [];
                  ...
                  children: [
                    Expanded(
                      child: _buildMemberInfoItem(
                        ...
                        'Join Date',
                        Text(
                          DateFormat('MMM d, yyyy').format(member.joinDate),
                          style: TextStyle(
                            fontSize: 14,
                            fontWeight: FontWeight.bold,
                            color: Theme.of(context).colorScheme.onSurface,
                ...
                if (member.documentUrl != null &&
                    member.documentUrl!.isNotEmpty)
                  Container(
                    padding: const EdgeInsets.all(8),
                    decoration: BoxDecoration(
                      borderRadius: BorderRadius.circular(12),
...
class _MemberRow extends ConsumerWidget {
  ...
  Widget _buildPlanBadge(BuildContext context, String plan) {
    Color planColor;
    if (plan.contains('Gold') || plan.contains('Annual'))
      planColor = Colors.amber;
    else if (plan.contains('Pro'))
      planColor = Colors.purple;
    else if (plan.contains('Student') || plan.contains('Silver'))
...
class _MemberCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    Color planColor;
    if (member.membershipPlan.contains('Gold') ||
        member.membershipPlan.contains('Annual'))
      planColor = Colors.amber;
    else if (member.membershipPlan.contains('Pro'))
      planColor = Colors.purple;
    else if (member.membershipPlan.contains('Student') ||
        member.membershipPlan.contains('Silver'))
      planColor = Colors.blueGrey;
    else
      planColor = Colors.blue;
